...
> **⚠️ 单一信号源（2026-06-21 起）**：WATCH→BUY 判定逻辑已抽到 `backtest/major_capital_rules.py`（`evaluate_bar` + `MajorCapitalParams` 验证默认值），由回测 `bt_major_capital.py._check_buy` 与实时/回填 `strategies.py.MajorCapitalAccumulationStrategy._signals` **共用**。
> 此前两者是**两套独立实现**，实时端跑的是 P0 之前的老逻辑（无 rsi≥55/break_high30，signal_f 开）——即所有 P 系列调参从未部署、线上信号/前向数据全是老策略（详见 memory: major-capital-oos-edge-and-dualtrack-gap）。
> **改信号逻辑只改 `major_capital_rules.py`。** 双闸回归：① bt 4 折须仍 +30.52%/逐笔不变；② strategies.py 须复现 6 个金标准 BUY（601456/300790/002881/000536/300310/605100，信号日=回测 buy_date−1）。
> **数据源已统一（2026-06-21）**：`strategy_analyzer._fetch_bars` 改用 `db.stock_dao.get_daily_history`（local_db，与回测 `load_all_db_data` 同表同口径），不再走 `BacktestDataLoader`（曾回退 akshare/腾讯致 qfq 漂移）。仅 db 无该股时退回联网+告警。
> **信号轨已重backfill（2026-06-21）**：删旧 86120 条老逻辑事件，公共核重跑 → 3431 事件（WATCH 3424 / BUY 7，老逻辑 BUY 曾 11.8k → **砍 96%**）。7 BUY = 6 金标准 + 1 额外（002577，信号轨不加大盘/槽位过滤）。pattern_tracker 已重算前向收益。
...
### P10 — trail 哲学复检：「越赚越放」在干净入场上是否反转？（验证后否决）
**问题**：P0#1 把"越赚越放"(k=3→5→7)反转为"越赚越紧"(k=2→1.5→1)，但那次 A/B 跑在【旧脏入场体系】上（signal_f 开、无 rsi≥55、无 break_high30）。盲点：现在入场已被过滤到 Fold2/3 闭仓 100% 胜率，对真正能走趋势的高质量突破，"让利润奔跑"是否反而更优？

**实验**：固定当前所有过滤/形态参数，仅切 trail 哲学，同次运行重测（TIGHT_CURRENT 重跑 = +30.52%，与文档基准吻合，对照可信）。脚本 `backtest/p10_trail_philosophy.py`。

**4 折 OOS 结果**：
| 臂 | 累计 | Fold2/3 胜率 | Fold2/3 MDD | 净 vs 现默认 |
|------|------|-------------|-------------|-------------|
| **TIGHT_CURRENT**（现默认 越赚越紧） | **+30.52%** | 100% / 100% | 4.45% / 5.23% | — |
| LOOSE_MID（温和放宽 2.5/3.0/4.0） | +28.06% | 75% / 75% | 10.5% / 10.6% | **-2.46pp** |
| LOOSE_OLD（海龟式 3.0/5.0/7.0） | +26.55% | 75% / 75% | 11.5% / 12.6% | **-3.96pp** |

**机制（Fold3 逐笔摊开）**：越赚越放被三个效应同时打击——
1. **明星单反而少赚**：300790 宇瞳光学 紧 +42.48% vs 放宽 +37.54%（回撤容忍 = 把浮盈吐回）
2. **槽位争用漏赢家**：loose 持仓久占满仓位槽，605100 华丰股份(+22.94%)信号来时无空槽 → 整笔没买到（loose 仅 6 笔 vs tight 8 笔）
3. **让输家多跌**：300310 宜通世纪 紧 +1.22% → 放宽 -4.68%

**结论**：盲点假设证伪。**越放越差且单调，MDD 翻倍**。唯一 Fold2 单看 loose 占优（+18.84% vs +14.16%），但被 Fold3 损失盖过，净额一致为负。本策略捕"主力建仓突破后一波"（数日~数周级），非海龟数月大趋势——该快进快出锁利。**不采纳，不动默认。**

**稳健性交叉验证（单段连续、非重叠、不重复计笔）**：换窗口构造重测,结论方向/量级不变——
| 连续段 | TIGHT | LOOSE_OLD | LOOSE_MID |
|--------|-------|-----------|-----------|
| 2024-09~2026-03 | +14.16% win100% mdd4.45% (9 笔) | +11.93% win80% mdd12.81% (8 笔) | +10.89% win80% mdd10.92% (8 笔) |
| 2025 全年 | 同上(完全相同) | 同上 | 同上 |

两段数字完全相同 → **2024-09~12 为 0 交易,全部成交落在 2025**(印证 P6/P7 dead zone)。换时段验证的硬限:**不存在另一个有交易的 regime**(2023/2024H1/2020-2022 全 0 交易),所有测试都基于同一批 2025 的 9 笔——只能靠前向真实样本扩充,无法靠历史换时段。

**数据**：`logs/p10_trail_philosophy.json`
